sap.ui.define([
...
], function (Controller, JSONModel, Fragment, WorkflowService, BusyDialog) {
	"use strict";
	...
	return Controller.extend("com.atc1.controller.Dashboard", {
		 ...
		 * @memberOf com.atc1.view.Dashboard
		 */
		onInit: function () {
			this._busyDialog = new BusyDialog(this.getView().getId() + "-busyDialog", {
				text: "Extracting file..."
			});

			//To get Workflow service token
		...
		createInvoice: function () {
			...
			if (!this.oUploadDialog) {
				Fragment.load({
					id: "uploadDialog",
					name: "com.atc1.view.fragments.InvoiceUploader",
					controller: this
		...
		onUploadComplete: function (oEvent) {
			...
			this._busyDialog.open()
				// Simulate file processing after 5 seconds
			setTimeout(function () {

				this._busyDialog.setText("Fetching file...");

				// Simulate more processing if needed
				setTimeout(function () {
					...
					WorkflowService.getData().then(function (oResponse) {
						...
						oResponse[0].TradeNumber = "T0011";
						oResponse[0].currentDate = "13/06/2024";
						oResponse[0].SenderName = "Telecommunications";
						oResponse[0].DocumentNumber = "Inv-1234";
				...
				}.bind(this), 3000); // Example: Fetching file might take 3 seconds
			}.bind(this), 5000); // Wait for 5 seconds to change the text
		...
		onRowClick: function (oEvent) {
			var selItem = oEvent.getSource().getBindingContext().getObject();
			this.getView().getModel().setProperty("/SelectedTrade", selItem)
		}
